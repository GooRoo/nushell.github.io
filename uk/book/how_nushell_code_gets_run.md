...
2. **_(Evaluate):_** Parses the commandline input
3. **_(Evaluate):_** Evaluates the commandline input
4. **_(Evaluate):_** Merges the environment (such as the current working directory) to the internal Nushell state
5. **_(Print):_** Displays the results (if non-`null`)
6. **_(Loop):_** Waits for another input
